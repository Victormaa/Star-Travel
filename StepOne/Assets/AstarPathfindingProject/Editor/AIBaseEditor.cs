...
namespace Pathfinding {
	...
	public class BaseAIEditor : EditorBase {
		protected SerializedProperty gravity, groundMask, centerOffset, acceleration, orientation, enableRotation, rotationSpeed, slowWhenNotFacingTarget;
		...
		protected override void OnEnable () {
			...
			slowWhenNotFacingTarget = serializedObject.FindProperty("slowWhenNotFacingTarget");
		}
		...
		protected override void Inspector () {
			...
			while (p.NextVisible(false)) {
				if (!SerializedProperty.EqualContents(p, groundMask) && !SerializedProperty.EqualContents(p, centerOffset) && !SerializedProperty.EqualContents(p, gravity) && !SerializedProperty.EqualContents(p, enableRotation) && !SerializedProperty.EqualContents(p, rotationSpeed) && !SerializedProperty.EqualContents(p, slowWhenNotFacingTarget) && !SerializedProperty.EqualContents(p, orientation)) {
					...
					if (SerializedProperty.EqualContents(p, acceleration)) {
