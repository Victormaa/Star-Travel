...
namespace Pathfinding.RVO {
	...
	public interface IAgent {
		 ...
		 *
		 * \note This is a Vector2, not a Vector3. The value is relative to the #MovementPlane.
		 *
		 ...
		 *
		 * \see #GraphTransform.xyPlane
		 * \see #GraphTransform.xzPlane
		 */
		 ...
		 * This determines where the base of the agent collision cylinder is relative to the #Position.
		 * If this is zero, then the base of the cylinder is at #Position, but you might for example want the position to be at the center of the cylinder.
		 * In that case you can set this to -0.5 * #Height.
		 */
		 ...
		 *
		 * \note The velocity will be projected on the current movement plane.
		 */
	...
	public class Simulator {
		...
		* \see DoubleBuffering */
		 ...
		 * Used internally by the simulation to perform fast neighbour lookups for each agent.
		 *
		 * #Quadree is used for planar movement and this KDTree is used for non-planar movement (e.g local avoidance on the surface of a sphere)
		 */
		//KDTree kdTree;
		...
		public IAgent AddAgent (Vector3 position) {
			return AddAgent(new Agent(position));
		}
		...
		public IAgent AddAgent (Vector2 position, float elevationCoordinate) {
			return AddAgent(new Agent(new Vector3(position.x, elevationCoordinate, position.y)));
		}
		...
		Vector3[] agentQuadtreePositions = new Vector3[0];
		void BuildQuadtree () {
			if (agentQuadtreePositions.Length < agents.Count) agentQuadtreePositions = new Vector3[agents.Count];
			Quadtree.Clear();
			Quadtree.threeD = true;
			if (agents.Count > 0) {
				for (int i = 0; i < agents.Count; i++) {
					agentQuadtreePositions[i] = agents[i].position;
				}

				var mn = agentQuadtreePositions[0];
				var mx = agentQuadtreePositions[0];
				for (int i = 1; i < agents.Count; i++) {
					mn = Vector3.Min(mn, agentQuadtreePositions[i]);
					mx = Vector3.Max(mx, agentQuadtreePositions[i]);
				}
				Quadtree.SetBounds(mn, mx);
		...
		public void Update () {
			...
			if (Time.time - lastStep >= DesiredDeltaTime) {
				...
				if (Multithreading) {
					...
					// TODO: Shouldn't quadtree run after the buffer switch? (uses position, not Position)
					BuildQuadtree();
