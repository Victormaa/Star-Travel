...
namespace Pathfinding.RVO.Sampled {
	...
	public class Agent : IAgent {
		...
		SimpleMovementPlane nextMovementPlane = new SimpleMovementPlane();
		SimpleMovementPlane resultMovementPlane = new SimpleMovementPlane();
		...
		/** \copydoc Pathfinding::RVO::IAgent::MovementPlane */
		public IMovementPlane MovementPlane { get { return resultMovementPlane; } }
		...
		/** \copydoc Pathfinding::RVO::IAgent::HeightOffset */
		...
		/** \copydoc Pathfinding::RVO::IAgent::SetMovementPlane */
		public void SetMovementPlane (IMovementPlane movementPlane) {
			movementPlane.CopyTo(nextMovementPlane);
		}
		...
		public void ForceSetVelocity (Vector3 velocity) {
			// A bit hacky with the *1000, but it is approximately correct
			// assuming the agent does not move significantly
			// The transformations are required to make things consistent with the rest of the simulation which
			// always projects all velocities on the movement plane.
			calculatedTargetPoint = movementPlane.ToPlane(position + velocity * 1000);
			nextTargetPoint = movementPlane.ToWorld(calculatedTargetPoint, localElevation);
		...
		public void BufferSwitch () {
			...
			radius = Radius;
			height = Height;
			Memory.Swap(ref movementPlane, ref nextMovementPlane);
			...
			position = Position;
			localPosition = movementPlane.ToPlane(Position, out localElevation);
			collidesWith = CollidesWith;
			layer = Layer;

			if (locked) {
				// Locked agents do not move at all
				desiredTargetPointInVelocitySpace = desiredVelocity = Vector2.zero;
				currentVelocity = Vector3.zero;
			} else {
				desiredTargetPointInVelocitySpace = movementPlane.ToPlane(nextTargetPoint) - localPosition;
				...
				// Note that we use MovementPlane and not movementPlane: the new movement plane may
				// be completely different from the movement plane used to calculate the last velocity.
				currentVelocity = MovementPlane.ToWorld(CalculatedTargetPoint - MovementPlane.ToPlane(position)).normalized * CalculatedSpeed;
				...
				desiredWorldVelocity = movementPlane.ToWorld(desiredVelocity, 0);

				if (collisionNormal != Vector3.zero) {
					collisionNormal.Normalize();
					var dot = Vector3.Dot(currentVelocity, collisionNormal);
		...
		public void PostCalculation () {
			...
			Util.Memory.Swap(ref movementPlane, ref resultMovementPlane);
		}
		...
		public void CalculateNeighbours () {
			...
			//	Debug.DrawLine(neighbours[i].position, position, Color.black);
			//}
		}
		...
		internal float InsertAgentNeighbour (Agent agent, float rangeSq) {
			...
			// 2D distance
			var dir = movementPlane.ToPlane(agent.position - position);
			float dist = dir.sqrMagnitude;

			// Check if the agent is within the threshold for considering it
			if (dist < rangeSq) {
		...
		Vector3 FromXZ (Vector2 p) {
			return movementPlane.ToWorld(p, localElevation);
		}
		...
		internal void CalculateVelocity (Pathfinding.RVO.Simulator.WorkerContext context) {
			...
			if (locked) {
				calculatedSpeed = 0;
				calculatedTargetPoint = localPosition;
				return;
			}
			...
			if (!insideAnyVO) {
				...
				calculatedTargetPoint = localPosition + desiredTargetPointInVelocitySpace;
			...
			result = GradientDescent(vos, movementPlane.ToPlane(currentVelocity), desiredVelocity);

			if (DebugDraw) Draw.Debug.CrossXZ(FromXZ(result+localPosition), Color.white);
			//Debug.DrawRay (To3D (position), To3D (result));

			calculatedTargetPoint = localPosition + result;
		...
		void GenerateObstacleVOs (VOBuffer vos) {
			...
			IMovementPlane plane = movementPlane;
			...
			for (int i = 0; i < simulator.obstacles.Count; i++) {
				...
				do {
					...
					var p1 = plane.ToPlane(vertex.position, out elevation1);
					var p2 = plane.ToPlane(vertex.next.position, out elevation2);

					Vector2 dir = (p2 - p1).normalized;

					// Signed distance from the line (not segment, lines are infinite)
					// TODO: Can be optimized
					float dist = VO.SignedDistanceFromLine(p1, dir, localPosition);

					if (dist >= -0.01f && dist < range) {
						float factorAlongSegment = Vector2.Dot(localPosition - p1, p2 - p1) / (p2 - p1).sqrMagnitude;
						...
						var sqrDistToSegment = (Vector2.Lerp(p1, p2, factorAlongSegment) - localPosition).sqrMagnitude;
						...
						if (sqrDistToSegment < range*range && (/* TODO simulator.movementPlane == MovementPlane.XY || */ (localElevation <= segmentY + vertex.height && localElevation + height >= segmentY))) {
							vos.Add(VO.SegmentObstacle(p2 - localPosition, p1 - localPosition, Vector2.zero, radius * 0.01f, 1f / ObstacleTimeHorizon, 1f / simulator.DeltaTime));
						}
					}
		...
		void GenerateNeighbourAgentVOs (VOBuffer vos) {
			...
			Vector2 optimalVelocity = movementPlane.ToPlane(currentVelocity);
			IMovementPlane plane = movementPlane;
			...
			for (int o = 0; o < neighbours.Count; o++) {
				...
				float otherElevation;
				var otherPosition = plane.ToPlane(other.position, out otherElevation);
				float maxY = System.Math.Min(localElevation + height, otherElevation + other.height);
				float minY = System.Math.Max(localElevation, otherElevation);
				...
				Vector2 voBoundingOrigin = otherPosition - localPosition;
				...
				Vector2 otherOptimalVelocity = plane.ToPlane(Vector3.Lerp(other.currentVelocity, other.desiredWorldVelocity, 2*avoidanceStrength - 1));
				...
				if (DebugDraw)
					DrawVO(localPosition + voBoundingOrigin * inverseAgentTimeHorizon + voCenter, totalRadius * inverseAgentTimeHorizon, localPosition + voCenter);
			}
		}
		...
		Vector2 GradientDescent (VOBuffer vos, Vector2 sampleAround1, Vector2 sampleAround2) {
			...
			if (DebugDraw) Draw.Debug.CrossXZ(FromXZ(minima1 + localPosition), Color.yellow, 0.5f);
			...
			if (DebugDraw) Draw.Debug.CrossXZ(FromXZ(minima2 + localPosition), Color.magenta, 0.5f);
		...
		Vector2 Trace (VOBuffer vos, Vector2 p, out float score) {
			...
			for (int s = 0; s < MaxIterations; s++) {
				...
				if (DebugDraw) Debug.DrawLine(FromXZ(prev + localPosition), FromXZ(p + localPosition), Rainbow(s*0.1f) * new Color(1, 1, 1, 1f));
			}
