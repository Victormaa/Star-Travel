...
namespace Pathfinding.RVO {
	 ...
	 *
	 * \snippet MiscSnippets.cs RVOController
	 *
	...
	public class RVOController : VersionedMonoBehaviour {
		...
		IMovementPlane nextMovementPlane;

		public void SetMovementPlane (IMovementPlane plane) {
			if (movementPlane != MovementPlane.Arbitrary) throw new System.InvalidOperationException("You can only set the movement plane when movementPlaneMode (alias for the movementPlane set on the RVOSimulator component) is set to Arbitrary");
			nextMovementPlane = plane;
		}
		...
		public Vector3 CalculateMovementDelta (float deltaTime) {
			...
			return rvoAgent.MovementPlane.ToWorld(Vector2.ClampMagnitude(rvoAgent.CalculatedTargetPoint - rvoAgent.MovementPlane.ToPlane(ai != null ? ai.position : tr.position), rvoAgent.CalculatedSpeed * deltaTime), 0);
		}
		...
		public Vector3 CalculateMovementDelta (Vector3 position, float deltaTime) {
			return rvoAgent.MovementPlane.ToWorld(Vector2.ClampMagnitude(rvoAgent.CalculatedTargetPoint - rvoAgent.MovementPlane.ToPlane(position), rvoAgent.CalculatedSpeed * deltaTime), 0);
		}
		...
		IMovementPlane agentMovementPlane {
			get {
				if (rvoAgent != null && rvoAgent.MovementPlane != null) return rvoAgent.MovementPlane;
				if (movementPlane == MovementPlane.XY) return GraphTransform.xyPlane;
				else return GraphTransform.xzPlane;
			}
		}
		...
		public Vector2 To2D (Vector3 p) {
			return agentMovementPlane.ToPlane(p);
		}
		...
		public Vector2 To2D (Vector3 p, out float elevation) {
			return agentMovementPlane.ToPlane(p, out elevation);
		}
		...
		public Vector3 To3D (Vector2 p, float elevationCoordinate) {
			return agentMovementPlane.ToWorld(p, elevationCoordinate);
		}
		...
		protected void UpdateAgentProperties () {
			...
			IMovementPlane plane;
			switch (movementPlane) {
			case MovementPlane.XY:
				plane = GraphTransform.xyPlane;
				break;
			case MovementPlane.XZ:
				plane = GraphTransform.xzPlane;
				break;
			default:
			case MovementPlane.Arbitrary:
				plane = nextMovementPlane ?? GraphTransform.xzPlane;
				break;
			}
			...
			// TODO: Movement plane used here is off by 1 frame
			rvoAgent.Position = (ai != null ? ai.position : tr.position) + plane.ToWorld(Vector2.zero, center - 0.5f * height);
			rvoAgent.SetMovementPlane(plane);
		}
		...
		public void Move (Vector3 vel) {
			...
			rvoAgent.SetTarget((ai != null ? ai.position : tr.position) + vel, speed, speed);
